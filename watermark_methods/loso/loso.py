...
LoSo is modified based on the code of MPAC:
...
class LOSOProcessor(WatermarkBase, LogitsProcessor):
...
class LOSODetector(WatermarkBase):
    def __init__(
        self,
        *args,
    ...
    def _predict_message(self, position_cnt, green_cnt_by_position, p_val_per_pos, num_candidates=16):
        ...
        for pos in range(1, self.converted_msg_length + 1):
            if position_cnt.get(pos) is None:
                ...
                pred, _ = min(enumerate(green_counts), key=lambda x: (x[1], x[0]))
